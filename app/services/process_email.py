...
# when executing this file directly (e.g. `python app/services/process_email.py`),
# Python sets sys.path[0] to the script's directory which is not the project root
# and the `app` package cannot be found.  Insert the workspace root on sys.path
# just like the other `process_*.py` helpers so absolute imports resolve.
...
def generate_email_digest(hours: int = 2400, top_n: int = 10) -> EmailDigestResponse:
...
def send_digest_email(hours: int = 2400, top_n: int = 10) -> dict:
    try:
...
if __name__ == "__main__":
    result = send_digest_email(hours=2400, top_n=10)
    if result["success"]:
        print("\n=== Email Digest Sent ===")
